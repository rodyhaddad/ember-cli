...
module.exports = Command.extend({
  name: 'init',
  aliases: ['i'],
  description: 'Creates a new project in the current folder.',
  works: 'everywhere',
  ...
  run: function(commandOptions, rawArgs) {
    ...
    if(!packageName) {
      var message = 'The `' + this.cli.name + ' ' + this.name + '` command requires a ' +
                    'package.json in current folder with name attribute or a specified name via arguments. ' +
                    'For more details, use `' + this.cli.name + ' help`.';

...
module.exports = Command.extend({
  name: 'new',
  description: 'Creates a new directory and runs ' + chalk.green('init') + ' in it.',
  ...
  run: function(commandOptions, rawArgs) {
    ...
    if (!packageName) {
      message = chalk.yellow('The `' + this.cli.name + ' ' + this.name + '` command requires a ' +
                             'name to be specified. For more details, use `' + this.cli.name + ' help`.');
    ...
    if (packageName === '.') {
      message = 'Trying to generate an application structure in this directory? Use `' + this.cli.name + ' init` instead.';
